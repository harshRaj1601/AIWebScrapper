...
if not db_initialized:
    st.error(
        "🚨 **Database is not configured!** This project requires a database to function."
    )
    ...
    3. **For production environments**, you can use PostgreSQL or MySQL:
    ...
    5. **Restart the project**, and you're good to go! 🚀
    """
    )
...
with st.sidebar.expander("API Keys", expanded=False):
    ...
    st.session_state["DATABASE_URL"] = st.text_input("DATABASE URL")
...
# Add max_output_tokens slider when Gemini is selected
...
if "gemini" in model_selection.lower():
    ...
    max_output_tokens = st.sidebar.slider(
        ...
        value=2048,
        step=100,
        help="Set the maximum number of tokens in the model's response."
    )
